...
export default function ExcelManager({ products }: { products: any[] }) {
  ...
  // --- FUNÇÃO DE EXPORTAR (Baixar Excel) ---
  const handleExport = () => {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        // CORREÇÃO DEFINITIVA: Convertemos para 'unknown[]' primeiro, depois iteramos
        // Isso evita o erro de sintaxe do TypeScript no loop
        const excelData = XLSX.utils.sheet_to_json(sheet);

        let count = 0;

        // Loop simples sem anotação de tipo dentro dos parênteses
        for (const item of excelData) { 
          // Forçamos o item a ser tratado como objeto qualquer aqui dentro
          const row = item as any;

          // Verificação de segurança: Se não tiver nome, pula
          ...
          // Chama a ação do servidor para salvar
        ...
        console.error(error);
        alert("Erro ao importar. Verifique se a planilha segue o modelo correto.");
      } finally {
