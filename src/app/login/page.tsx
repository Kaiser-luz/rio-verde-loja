...
import { supabase } from '@/lib/supabase'; // Importa conexão real
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // Sucesso! O AuthContext vai detectar a mudança automaticamente
        router.push('/'); 
        router.refresh();
      }
    } catch (err) {
    ...
    <div className="min-h-[80vh] flex items-center justify-center bg-stone-50 px-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-stone-100 animate-fade-in">
        <div className="text-center mb-8">
          ...
          <p className="text-stone-500">Aceda à sua conta Rio Verde</p>
        </div>
