...
interface CreateProfileData {
    userId: string; name: string; email: string; cpf: string; phone: string;
    role: string; cnpj?: string; zipCode?: string; street?: string; number?: string;
...
export async function createProfile(data: CreateProfileData) {
    ...
    await prisma.profile.create({
        data: {
            ...
            role: data.role, cnpj: data.cnpj, approved: isApproved,
            zipCode: data.zipCode, street: data.street, number: data.number,
...
export async function createOrder(
    cartItems: any[], itemsTotal: number, customerName: string, userId?: string,
    shippingCost: number = 0, deliveryMethod: string = 'retirada_loja'
) {
    let shippingData = {};
    if (userId) {
        ...
        if (profile) {
            shippingData = {
                shippingZipCode: profile.zipCode, shippingStreet: profile.street, shippingNumber: profile.number,
                shippingComplement: profile.complement, shippingDistrict: profile.district, shippingCity: profile.city, shippingState: profile.state,
            };
        }
    }
    ...
    const order = await prisma.order.create({
        data: {
            customer: customerName || "Visitante", userId: userId || null, total: finalTotal, status: "pendente",
            shippingCost: shippingCost, deliveryMethod: deliveryMethod, ...shippingData,
            items: {
                create: cartItems.map((item: any) => ({
                    productName: item.name, price: item.price, quantity: item.quantity, color: item.selectedColor.name
                }))
            }
        }
    });
...
export async function createProduct(formData: FormData) {
    ...
    await prisma.product.create({
        data: {
            name, price, priceUpholsterer, stock, category: categoryId, type: category.type, image, colors, pdfUrl: pdfUrl || null,
            width, weight, composition: composition || null
        },
    });
