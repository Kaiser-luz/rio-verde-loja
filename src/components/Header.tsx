...
export default function Header() {
  ...
  const { user, signOut, isLoading } = useAuth(); // signOut em vez de logout
  ...
  // Função auxiliar para pegar o nome do usuário
  const getUserName = () => {
    if (!user) return '';
    // Tenta pegar dos metadados (onde salvamos no cadastro) ou usa o começo do email
    return user.user_metadata?.full_name?.split(' ')[0] || user.email?.split('@')[0];
  };
