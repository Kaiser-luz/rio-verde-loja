...
export default function CartSidebar() {
  ...
  // Função auxiliar para pegar o nome legível
  const getCustomerName = () => {
    if (!user) return "Visitante";
    // Tenta pegar o nome dos metadados (salvo no cadastro) ou o email
    return user.user_metadata?.full_name || user.email || "Cliente Logado";
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // 1. Salva o pedido no Banco de Dados com o NOME DO CLIENTE
      const orderId = await createOrder(cart, cartTotal, customerName);
      ...
      message += `*Cliente:* ${customerName}\n`; // Nome correto na mensagem também
      message += `-----------------------------------\n\n`;
